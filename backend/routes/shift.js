		...
		.then(async (newShift) => {
		 ...
		 let created =	req.body.superadmin ? await Super.findOne({_id : req.body.Created}) : await User.findOne({_id : req.body.Created})
			...
			var mailOptions = {
				...
				//isSuper==true ? '<p> The '+ shift.shiftname +' for ' +  newShift.start  +  ' has been created for '+ user.lastName + ", " + user.firstName.charAt(0)  +   ' by ' + created.first_name.charAt(0) + " " + created.last_name + '</p>':
				html:'<p> The '+ shift.shiftname +' call for ' +  newShift.start  +  ' has been created for '+ user.lastName + ", " + user.firstName.charAt(0)  +   ' by ' + created.lastName + ", " + created.firstName.charAt(0)+'</p>',
			};
					...
					if (shift.shiftTypeId && shift.shiftTypeId.shiftname === 'Request') {
						...
						if (shift.comment && shift.comment.length > 8) {
							return {
								...
								start: shift.start,
								priority: shift.shiftTypeId ?shift.shiftTypeId.priority :null,
								end: shift.end,
								shiftname: shift.shiftTypeId ?shift.shiftTypeId.shiftname:null,
								requestApprovalStatus: shift.requestApprovalStatus,
								title: shift.shiftTypeId ?shift.shiftTypeId.shiftname.substring(0, 3) + ':' + ' ' + shift.userId.lastName + ' (' + shift.comment.substring(0, 14) + '...' + ')' : null,
								color:  shift.shiftTypeId ? ((count > 8 && shift.shiftTypeId.shiftname == "Off") ? 'red' : shift.shiftTypeId.color):null,
								swapable: shift.swapable,
								userId: shift.userId._id,
								userType: shift.userId.type,
								comment: shift.comment,
							...
							return {
								...
								shiftname: shift.shiftTypeId ?shift.shiftTypeId.shiftname :null,
								requestApprovalStatus: shift.requestApprovalStatus,
								title:shift.shiftTypeId ? shift.comment ? shift.shiftTypeId.shiftname.substring(0, 3) + ':' + ' ' + shift.userId.lastName + ' (' + shift.comment + ')' : shift.shiftTypeId.shiftname.substring(0, 3) + ':' + ' ' + shift.userId.lastName + ', ' + shift.userId.firstName:null,
								color:  shift.shiftTypeId ? ((count > 8 && shift.shiftTypeId.shiftname=="Off") ? 'red' : shift.shiftTypeId.color):null,
								swapable: shift.swapable,
								userId: shift.userId._id,
								userType: shift.userId.type,
						...
						return {
							...
							shiftname:shift.shiftTypeId ? shift.shiftTypeId.shiftname : null,
							requestApprovalStatus: shift.requestApprovalStatus,
							title: shift.shiftTypeId ? shift.shiftTypeId.shiftname + ':' + ' ' + shift.userId.firstName.charAt(0) + ' ' + shift.userId.lastName : null,
