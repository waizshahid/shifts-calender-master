...
const Admin = ({ admin }) => {
      ...
      .then((res1) => {
        ...
        for (let i = 0; i < res1.data.length; i++) {
          if (res1.data[i].requesterType === "Super Admin") {
            ...
            res1.data[i].requesterType === "Admin" &&
            res1.data[i].currentUserId === currentId
          ) {
            array.push(res1.data[i]);
          }
          else if (
            res1.data[i].requesterType === "User" &&
            res1.data[i].from === currentId
          ) {
            array.push(res1.data[i]);
          }
        }
  ...
  const showShiftModal1 = (message) => {
    ...
    {
      dsplayMessage[message].requesterType == "User"
        ? set2Visible(true)
        : setVisible(false);
    }
    const editingProfile = () => {
    ...
    axios
      ...
      .then((res1) => {
        axios
          .get("user/getShiftFrom/" + dsplayMessage[message].shiftFrom)
          .then((res2) => {
            axios
  ...
  const showShiftModal = (message) => {
    ...
    axios
      ...
      .then((res1) => {
        axios
          .get("user/getShiftFrom/" + dsplayMessage[message.key].shiftFrom)
          .then((res2) => {
            axios
        ...
        <Menu.Item key={index}>
          ...
          >
            {message.requesterType === "Super Admin" ? (
              <div>
                <div className="row">
                <Tag className="col-2" color="success" onClick={() =>showShiftModal1(index)}>Detail</Tag>
                  <div
                    className="col-6"
                    style={{
                      display: "flex",
                ...
                {message.currentUserId === currentId ? (
                  <div>
                    ...
                    <div  style={{ margin: 2 }} className="row">
                      {"Your request for the shift named " +
                        message.shiftName +
                        " has been sent"}
                    </div>
                    <div>
              ...
              <div>
                {message.requesterType === "User" ? (
                  <div className="">
                    ...
                    <Tag color="success" onClick={() =>showShiftModal1(index)}>Detail</Tag>
                        ...
                        <Row className="buttonsetting" >
                              ...
                              onClick={() =>deleteNotification1(index)}
                              >Delete this notification</Button>
                              <Button 
                              onClick={() =>
                                  updateNotification1(index)
                                 } 
                                className="rejectbutton" >Reject</Button>
                              <Button type="primary" className="button2setting" 
                               onClick={()=>swapShift1(index)}
                              >Exchange</Button>
                              </Row> <hr />
                      </div>) : (<div>
                      <div>
                    ...
                    <div className="row">
                    <div className="col-2"> <Tag color="success" onClick={() =>showShiftModal1(index)}>Detail</Tag>  </div>
                      <div className="col-4">
                        <Tag color="success">{message.requesterType}</Tag>
                      </div>
                      <div className="col-2">
    ...
    <div>
          ...
          <div style={{ backgroundColor: "black" }}>
            <Sider style={{ height: "100vh" }}>
              <h5 className="pt-4 pb-2 text-center text-muted">
                {admin.firstName.charAt(0) + " " + admin.lastName}
              </h5>
      ...
      >
            ...
            {dsplayMessage.length === 0 ? (
              <span>
                ...
                >
                  ...
                  >
                    {console.log(admin)}
                    {admin.firstName.split(" ")[0].charAt(0).toUpperCase() +
                      admin.lastName.split(" ")[0].charAt(0).toUpperCase()}
                  </Avatar>
                </Dropdown>
              </span>
            ) : (
        ...
        <Switch>
          ...
          <Route
            exact
            path="/admin/shifts-calender"
            component={ShiftsCalender}
            userObj={myDetails}
          />
          <Route exact path="/admin/manage-users" component={ManageUsers} />
          <Route
            exact
            path="/admin/exchange-shifts"
      ...
      >
        <EditUser
          setEditProfile={(val) => setEditProfile(val)}
          userObj={myDetails}
          id={myDetails._id}
        />
      </Modal>
      ...
      >
        <Row>
          {/* <Col xs={24} sm={24} md={24} lg={24} xl={24}>
                       ...
                       */}
          <div>
            Swapping <b>{shiftname + " "}</b> calls for <b> {date + " "}</b>{" "}
            from <b>{user1 + " "}</b> to <b>{user2}</b>
          </div>
        </Row>
      </Modal>
      <Modal
                ...
                {index === 0 ? (
                  <Card type="inner">
                    <div>
                      ...
                      <br />
                      {"Name:" + " " + shifts[2].firstName + " " + shifts[2].lastName}
                      <br />
                      {"Email:" + " " + dat.email}
                      <br />
                    </div>
